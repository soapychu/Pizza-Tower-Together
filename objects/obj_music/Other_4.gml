...
if (!global.panic or global.leveltosave == "dragonlair")
{
	...
	{
		if check_sugary()
		pillarmusicID = fmod_event_create_instance("event:/music/harrymusic")
		else
...
if (secret)
{
	if (music != -4 && music.event_secret != -4)
	{
		...
		{
			case "SP": secstate = 1; break;
			case "G": if global.pizzellesugaryoverride secstate = 1 else secstate = 0 break;
			case "BN": secstate = 2; break;
			default: secstate = 0; break;
		}
		
		fmod_event_instance_set_parameter(music.event_secret, "state", secstate, true);
