function scr_painter_init_sounds()
{
...
function scr_painter_destroy_sounds()
{
	destroy_sounds([snd_shoulderbash, snd_groundpound]);
}
function scr_painter_sounds()
{
...
function scr_painter_spawn_knight()
{
	if (knightdir == -1)
	{
...
function scr_painter_shuffle_adds()
{
...
function scr_painter_arenaintro()
{
	if (!skipintro)
	{
		image_speed = 0.35;
		...
		scr_boss_genericintro(spr_painter_idle);
}
function scr_painter_walk()
{
	if (grounded)
		hsp = Approach(hsp, 0, 0.25);
	if (grounded && vsp > 0 && sprite_index == spr_painter_tired && flickertime > 2)
	{
	...
	if (cooldown > 0 && flickertime <= 0 && obj_player1.state != states.animation)
		cooldown--;
	if (flickertime > 0)
		sprite_index = spr_painter_tired;
	else if (grounded)
	{
		if (phase == 2)
			sprite_index = spr_painter_tired;
	...
	else if (stomp && cooldown == 0)
	{
		...
		hsp = image_xscale * 15;
		vsp = -16;
		sprite_index = spr_painter_idle; // jump
		image_index = 0;
	}
}
function painter_nearestspot()
{
	if (elitehit > 1)
	{
...
function scr_painter_jump()
{
	...
	if ((x >= (targetspot.x - 32) && x <= (targetspot.x + 32) && y < (targetspot.y - 48)) || (image_xscale > 0 && x > (targetspot.x + 100)) || (image_xscale < 0 && x < (targetspot.x - 100)) || vsp > 2)
	{
		...
		sprite_index = spr_painter_groundpoundstart;
		image_index = 0;
		vsp = 11;
		hsp = 0;
		grounded = false;
	}
	if (sprite_index == spr_painter_jump && floor(image_index) == (image_number - 1))
		sprite_index = spr_painter_fall;
}
function scr_painter_freefallprep()
{
	hsp = 0;
	vsp = 0;
	image_speed = 0.35;
	if (floor(image_index) == (image_number - 1))
	{
		...
		sprite_index = spr_painter_groundpound;
		grounded = false;
		vsp = 10;
	}
}
function scr_painter_freefall()
{
	...
	if (grounded)
	{
		...
		if (_land)
		{
			...
			sprite_index = spr_painter_groundpoundland;
			image_index = 0;
			landbuffer = 100;
			if (bowlingballcount > 0)
				bowlingballcount--;
			if (!pizzahead)
			{
				if (phase == 1)
				{
					...
					{
						bowlingballcount = MAX_bowlingballcount;
						with (instance_create(x, y, obj_painterbowlingballspawner))
							balls = 1;
					}
				}
				if (phase == 2)
					scr_painter_shuffle_adds();
			}
			with (obj_camera)
			{
				shake_mag = 3;
...
function scr_painter_rolling()
{
	if (sprite_index != spr_painter_groundpoundland)
	{
		image_speed = 0.35;
		sprite_index = spr_painter_rolling;
		hsp = image_xscale * 10;
		if (place_meeting(x + hsp, y, obj_solid) && !place_meeting(x + hsp, y, obj_slope_parent) && !place_meeting(x + hsp, y, obj_destructibles))
		{
			...
			image_xscale *= -1;
			vsp = -10;
			fmod_event_one_shot_3d("event:/sfx/painter/groundpoundbump", x, y);
		}
		if (grounded && vsp > 0)
		{
			state = states.walk;
			sprite_index = spr_painter_move;
			hsp = image_xscale * 5;
		}
	}
	else
...
function scr_painter_freefallland()
{
	hsp = 0;
	image_speed = 0.35;
	if (landbuffer > 0)
...
function scr_painter_shoulderbash()
{
	...
	if (floor(image_index) == (image_number - 1) && sprite_index == spr_painter_dashstart)
		sprite_index = spr_painter_dash;
}
function scr_painter_do_contemplate()
{
	if (wastedhits >= 8)
	{
		var b = false;
		...
		if (b)
		{
			if (state != states.contemplate)
				fmod_event_one_shot_3d("event:/sfx/voice/paintersnicker", x, y);
			state = states.contemplate;
			animbuffer = 80;
			hsp = 0;
			vsp = 0;
			sprite_index = spr_painter_contemplate;
		}
	}
}
function scr_painter_contemplate()
{
	hsp = 0;
	image_speed = 0.35;
}
function scr_painter_shoulderturn()
{
	if (sprite_index != spr_painter_shoulderturn)
		attackspeed = Approach(attackspeed, 0, 0.5);
	else
		attackspeed = Approach(attackspeed, 0, 0.1);
	hsp = image_xscale * attackspeed;
	if (sprite_index == spr_painter_turn && floor(image_index) == (image_number - 1))
		sprite_index = spr_painter_turnloop;
	if (attackspeed <= 0 && sprite_index == spr_painter_turnloop && floor(image_index) == (image_number - 1))
	{
		if (phase == 2)
		{
			attackspeed = 24;
...
function scr_painter_phase1hurt()
{
	sprite_index = spr_painter_hurt;
	image_speed = 0.35;
